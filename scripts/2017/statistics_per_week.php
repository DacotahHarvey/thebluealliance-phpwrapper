<?php
    ...
    // event into the code on line 48 (look for instances of $event)
    $target_week = null;
    if (isset($argv[1])) {
        $target_week = $argv[1];
    } else {
        print_r(
            ...
            "php statistics_per_week.php {week_number}"
        );
        die();
    }
    ...
    foreach ($events as $event) {
        ...
        // If the event is in the week that we are currently looking for then we
        // can pull it's data to include in the spreadsheet
        if ($event->week == $target_week) {

            // Fetch all of the events that we want to fetch for a given year
            $matches = $tbaRequest->getEventMatches(['event_key' => $event->key]);
